...
try:
    ...
    st.set_page_config(page_title="Roafy Music", page_icon=logo, layout="wide", initial_sidebar_state="expanded")
except Exception:
    st.set_page_config(page_title="Roafy Music", page_icon="🎵", layout="wide", initial_sidebar_state="expanded")
...
<style>
    ...
    div.stButton > button:hover { background-color: #1ed760 !important; transform: scale(1.03); }
    .song-card { background: #1e1e1e; border-radius: 10px; padding: 14px 18px; margin-bottom: 8px; transition: background 0.2s; }
    ...
    .player-card { background: #181818; border-radius: 12px; padding: 15px; margin-bottom: 20px; border: 1px solid #282828; }
...
def cargar_cola_completa(canciones, empezar_a_reproducir=False):
    """Carga canciones a la cola sin duplicar. Si empezar_a_reproducir=True, inicia la primera."""
    ids_existentes = {c["id"] for c in st.session_state.cola_reproduccion}
    agregadas = 0
    primera_nueva = None
    for c in canciones:
        if c["id"] not in ids_existentes:
            if primera_nueva is None:
                primera_nueva = c
            st.session_state.cola_reproduccion.append(c)
            ids_existentes.add(c["id"])
            agregadas += 1
    if empezar_a_reproducir and primera_nueva and not st.session_state.cancion_actual:
...
def limpiar_cola():
    ...
    st.session_state.reproductor_activo  = False
    st.toast("🗑️ Cola limpiada")
    st.rerun()

# ── REPRODUCTOR PERSISTENTE ───────────────────────────────────────────────────
# El reproductor siempre se muestra en la parte superior de la página activa.
# La clave es que toda la playlist viaja dentro del iframe en JSON,
# por lo que cambiar de página en Python NO detiene el audio —
# el iframe es destruido y recreado, PERO como el video_id y el índice
# se guardan en session_state, el reproductor retoma desde la canción correcta.
# Para reproducción 100% continua sin interrupción necesitaríamos una arquitectura
# con iframe fijo fuera del flujo de Streamlit (no soportado en Streamlit estándar).
# La solución aquí: el reproductor recuerda qué canción estaba sonando y la reinicia.
def mostrar_reproductor_avanzado():
    inicializar_cola()

    if not st.session_state.cancion_actual:
        return  # No mostrar nada si no hay canción activa

    c      = st.session_state.cancion_actual
    cola   = st.session_state.cola_reproduccion
    indice = st.session_state.indice_actual

    # Solo canciones con video válido
    playlist_data = []
    for cancion in cola:
    ...
    with col_video:
        ...
        <div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
            <button onclick="anteriorCancion()"
                style="background:#1DB954;color:#000;border:none;padding:8px 20px;
                       border-radius:500px;font-weight:700;cursor:pointer;font-size:15px;">⏮️ Anterior</button>
            <button onclick="siguienteCancion()"
                style="background:#1DB954;color:#000;border:none;padding:8px 20px;
                       border-radius:500px;font-weight:700;cursor:pointer;font-size:15px;">⏭️ Siguiente</button>
            <span id="badge_idx" style="color:#1DB954;font-weight:700;font-size:14px;margin-left:8px;">
                {indice_js + 1} / {len(playlist_data)}
            </span>
        </div>

        <div style="margin-top:8px;padding:10px;background:#1e1e1e;border-radius:8px;">
            <div id="titulo_actual" style="color:#FFFFFF;font-weight:700;font-size:15px;">{c.get('titulo','')}</div>
            <div id="artista_actual" style="color:#b3b3b3;font-size:13px;margin-top:4px;">{c.get('artista','')} · {c.get('genero','')}</div>
        </div>
        ...
        <script>
            ...
            function onYouTubeIframeAPIReady() {{
                ...
                player = new YT.Player('player', {{
                    ...
                    events: {{
                        ...
                        onError:       function(e) {{ setTimeout(siguienteCancion, 1500); }}
                    }}
                }});
            }}
            ...
            function cargarCancion(idx) {{
                ...
                if (!item || !item.videoId) {{ siguienteCancion(); return; }}
                currentIndex = idx;
    ...
    with col_info:
        sin_link = len(cola) - len(playlist_data)
        st.markdown(f"""
        <div class='player-card'>
            ...
            <p style='color:#555;font-size:0.75rem;margin:8px 0 0'>
                📋 {len(playlist_data)} canciones válidas en cola
                {'· ⚠️ ' + str(sin_link) + ' sin link' if sin_link > 0 else ''}
            </p>
        </div>
        """, unsafe_allow_html=True)
...
# ── ESTADOS GLOBALES ──────────────────────────────────────────────────────────
for k, v in [("logueado", False), ("user", None), ("cancion_actual", None),
             ("favoritos_ids", set()), ("pagina", "inicio"),
             ("cola_inicializada", False), ("reproductor_activo", False)]:
    if k not in st.session_state:
        st.session_state[k] = v
...
def tarjeta_cancion(cancion, idx, mostrar_fav=True):
    ...
    with col_play:
        if st.button("▶", key=f"play_{cid}_{idx}"):
            # Agregar a cola y reproducir — NO interrumpe si ya hay algo sonando
            agregar_a_cola(cancion)
            if not st.session_state.cancion_actual:
                st.session_state.cancion_actual = cancion
                st.session_state.indice_actual  = len(st.session_state.cola_reproduccion) - 1
...
def mostrar_sidebar():
    with st.sidebar:
        ...
        # Mini reproductor en sidebar para saber qué suena mientras navegas
        if st.session_state.cancion_actual:
            c = st.session_state.cancion_actual
            st.markdown(f"""
            <div style='background:#1e1e1e;border-radius:8px;padding:8px 12px;margin:8px 0;border-left:3px solid #1DB954;'>
                <p style='color:#1DB954;font-size:0.7rem;margin:0'>▶ SONANDO</p>
                <p style='color:#fff;font-size:0.82rem;font-weight:700;margin:2px 0'>{c.get('titulo','')}</p>
                <p style='color:#b3b3b3;font-size:0.75rem;margin:0'>{c.get('artista','')}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        for g in ["Rock", "Pop", "Urbano", "Tropical", "Electrónica", "Regional"]:
            if st.button(f"🎸 {g}", use_container_width=True, key=f"gen_{g}"):
                ...
                st.session_state.genero_sidebar = g
                st.rerun()
        ...
        if st.button("🚪 Cerrar sesión", use_container_width=True):
            for k in ["logueado","user","cancion_actual","cola_reproduccion",
                      "favoritos_ids","cola_inicializada","reproductor_activo"]:
                ...
                elif k in ("cola_inicializada","reproductor_activo"): st.session_state[k] = False
...
def pagina_inicio():
    ...
    # Auto-cargar cola la primera vez que entra
    if not st.session_state.cola_inicializada and canciones:
        cargar_cola_completa(canciones, empezar_a_reproducir=True)
        st.session_state.cola_inicializada = True

    # Mostrar reproductor (si hay algo sonando)
    ...
    with col_acc2:
        if st.button("🔀 Reproducir en aleatorio", use_container_width=True):
            if canciones:
                ...
                st.session_state.cola_reproduccion = []
                st.session_state.cola_inicializada = False
                cargar_cola_completa(mezcladas, empezar_a_reproducir=True)
                st.session_state.cola_inicializada = True
                st.rerun()
...
def pagina_buscar():
    ...
    # Si viene del sidebar con un género preseleccionado
    genero_default = st.session_state.pop("genero_sidebar", "Todos") if "genero_sidebar" in st.session_state else "Todos"

    col1, col2 = st.columns(2)
    with col1:
        opciones_genero = ["Todos", "Electrónica", "Pop", "Regional", "Rock", "Tropical", "Urbano"]
        idx_default     = opciones_genero.index(genero_default) if genero_default in opciones_genero else 0
        genero_filtro   = st.selectbox("🎸 Género", opciones_genero, index=idx_default)
    with col2:
    ...
    # Si buscó por texto, también sugerir canciones del mismo género aunque no coincidan con el texto
    canciones_sugeridas = []
    if query and todos:
        generos_encontrados = list({c.get("genero") for c in todos if c.get("genero")})
        ids_actuales        = {c["id"] for c in todos}
        for g in generos_encontrados:
            extras = supabase.table("canciones").select("*").eq("genero", g).execute().data or []
            for e in extras:
                if e["id"] not in ids_actuales:
                    canciones_sugeridas.append(e)
                    ids_actuales.add(e["id"])
        canciones_sugeridas = canciones_sugeridas[:10]

    # Botón para agregar resultados a la cola y reproducir
    if todos:
        if st.button(f"▶️ Reproducir estos {len(todos)} resultados", use_container_width=True):
            n = cargar_cola_completa(todos, empezar_a_reproducir=True)
            # También agregar sugeridas
            if canciones_sugeridas:
                cargar_cola_completa(canciones_sugeridas)
            st.toast(f"✅ {n} canciones agregadas a la cola")
            st.rerun()

    # Reproductor siempre visible (sigue sonando)
    mostrar_reproductor_avanzado()

    for i, c in enumerate(todos):
        tarjeta_cancion(c, f"bus_{i}")

    # Mostrar sugerencias del mismo género
    if canciones_sugeridas:
        st.markdown("---")
        st.markdown(f"### 💡 Más canciones del mismo género que podrían gustarte")
        for i, c in enumerate(canciones_sugeridas):
            tarjeta_cancion(c, f"sug_{i}")
...
def pagina_perfil():
    ...
    # Reproductor visible también en perfil (para que no se pierda el contexto)
    mostrar_reproductor_avanzado()
